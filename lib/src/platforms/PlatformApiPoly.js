...
var superspawn = require('judpack-common').superspawn;
var xmlHelpers = require('judpack-common').xmlHelpers;
var knownPlatforms = require('./platforms');
var CordovaError = require('judpack-common').CordovaError;
var PluginInfo = require('judpack-common').PluginInfo;
var ConfigParser = require('judpack-common').ConfigParser;
var PlatformJson = require('judpack-common').PlatformJson;
var ActionStack = require('judpack-common').ActionStack;
var PlatformMunger = require('judpack-common').ConfigChanges.PlatformMunger;
var PluginInfoProvider = require('judpack-common').PluginInfoProvider;
...
function PlatformApiPoly(platform, platformRootDir, events) {
    ...
    this.events = events || require('judpack-common').events;
...
PlatformApiPoly.createPlatform = function (destinationDir, projectConfig, options) {
    ...
    return Q().then(function(){
        ...
        if(options.ali&&options.platformDetails.platform == "ios"){
            var judPluginRootDir = path.join(destinationDir, 'Judplugin');
            changeSource(destinationDir)
            changeSource(judPluginRootDir)
        }
...
function changeSource (destinationDir){
    ...
    var judPluginRootDir = path.join(destinationDir);
    var xcodeProjDir;
    var xcodeCordovaProj;

    try {
        xcodeProjDir = fs.readdirSync(judPluginRootDir).filter( function(e) { return e.match(/\.xcodeproj$/i); })[0];
        if (!xcodeProjDir) {
            throw new CordovaError('The provided path "' + judPluginRootDir + '" is not a Jud iOS project.');
        }

        var cordovaProjName = xcodeProjDir.substring(xcodeProjDir.lastIndexOf(path.sep)+1, xcodeProjDir.indexOf('.xcodeproj'));
        xcodeCordovaProj = path.join(judPluginRootDir, cordovaProjName);
    } catch(e) {
        throw new CordovaError('The provided path "'+judPluginRootDir+'" is not a judpack iOS project.');
    }
    var Podfile = require('./ios_pack/lib/Podfile').Podfile;
    var project_name = xcodeCordovaProj.split('/').pop();
    var podfileFile = new Podfile(path.join(judPluginRootDir, Podfile.FILENAME), project_name)

...
var CordovaLogger = require('judpack-common').CordovaLogger;
var events = require('judpack-common').events;
var PluginManager = require('judpack-common').PluginManager;
var Q = require('q');
...
function Api(platform, platformRootDir, events) {
    ...
    var judPluginRootDir = path.join(platformRootDir, 'Judplugin');
  if(cordovaUtil.isJudPluginRoot(platformRootDir)){
    judPluginRootDir = platformRootDir
  }

    this.root = judPluginRootDir || path.resolve(__dirname, '..');
    ...
    try {
        ...
        if (!xcodeProjDir) {
            throw new CordovaError('The provided path "' + this.root + '" is not a Jud iOS project.');
        }
    ...
    this.locations = {
        ...
        //judpack modify it
        configXml: path.join(xcodeCordovaProj, 'Resources/JudpluginConfig.xml'),

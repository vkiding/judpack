...
var cordova_lib = require('../lib'),
    ...
    JudpackError = cordova_lib.CordovaError,
    cordova = cordova_lib.cordova,
    events = cordova_lib.events,
    logger = require('judpack-common').CordovaLogger.get();
...
function handleTelemetryCmd(subcommand, isOptedIn) {
    ...
    try {
        if (turnOn) {
            telemetry.turnOn();
            console.log("Thanks for opting into telemetry to help us improve judpack.");
        } else {
            telemetry.turnOff();
            console.log("You have been opted out of telemetry. To change this, run: judpack telemetry on.");
        }
    } catch (ex) {
        cmdSuccess = false;
    }
...
function cli(inputArgs) {
    ...
    if ( !cordova.hasOwnProperty(cmd) ) {
        msg =
            'judpack doesn\'t know command: ' + cmd + '; try ` judpack help` for a list of all the available commands.';
        throw new JudpackError(msg);
    }
    ...
    if (cmdList.indexOf(cmd)>=0) {
        ...
        if (args.variable) {
            args.variable.forEach(function (s) {
                ...
                var eq = s.indexOf('=');
                if (eq == -1)
                    throw new JudpackError("invalid variable format: " + s);

    ...
    npm.load(function(){
      var load = function(npmName){
          ...
          npm.commands.info([npmName+"@"+version], true, function(error,result){
            ...
            if(error&&trynum==0){
              trynum++;
              if(npmName == "jud-gcanvas"){
               var  prefix = "jud-plugin--"
              }
              else {
               var  prefix = "jud-plugin-"
              }
              ...
              var judpackVersion = result[version].judpack ;

              if(judpackVersion&&judpackVersion == "0.4.0"){
                callback({
                  ios: result[version].ios,
                  android: result[version].android,
                  version:result[version].version,
                  name:result[version].name,
                  judpack:result[version].judpack,

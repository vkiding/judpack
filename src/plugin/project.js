var config            = require('../../lib/src/cordova/config'),
    ...
    superspawn        = require('judpack-common').superspawn,
    semver            = require('semver'),
    shell             = require('shelljs'),
    _                 = require('underscore'),
    PlatformJson      = require('judpack-common').PlatformJson,
...
project.createProject = function(projectRoot, platform, opts) {
  ...
  return Q().then(function() {
    ...
    var platformPath = path.join(projectRoot, 'Judplugin');
    ...
    console.log('create judplugin project  success...')
...
function createProjectInJudpack() {
...
function downloadProject(projectRoot, platform) {
  return Q().then(function() {
    var git_url = 'https://github.com/judteam/judpluginContainer-iOS.git'
    if (platform == "ios") {
      git_url = 'https://github.com/judteam/judpluginContainer-iOS.git'
    }


    if(platform == "android") {
      git_url = 'https://github.com/judteam/judplugin-android.git'
    }
...
function cordova_git(platform) {
  ...
  if (!(platform.name in platforms)) {
    return Q.reject(new Error('judpack library "' + platform.name + '" not recognized.'));
  }

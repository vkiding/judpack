 ...
 */
const PluginInfo = require('judpack-common').PluginInfo;
...
module.exports = function (ali) {
  ...
  try {
    ...
    if (e.message.indexOf('Cannot find plugin.xml for plugin') != -1) {
      console.log();
      console.log(Chalk.red('  jud plugin project not found !'));
      console.log();
      console.log(`  You should run ${Chalk.blue('judpack plugin create')} first`);
      console.log();
      process.exit();
    }
    else {
...
function _doPublish(package, name, namespace, fullname, ali, deps,extend) {
  ...
  if (deps.length > 0) {
    package.scripts = {
      'postinstall': 'node ./judpack-hook.js'
    };
    let source = Fs.readFileSync(Path.join(__dirname, 'judpack-hook.js')).toString();
    let code = 'var deps=' + JSON.stringify(deps) + ';\n';
    if (name !== fullname) {
      code += 'var self=' + JSON.stringify({name, fullname}) + ';\n';
    }
    Fs.writeFileSync('./judpack-hook.js', code + source)
  }

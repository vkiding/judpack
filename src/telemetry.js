...
function showPrompt() {
    ...
    var msg = "May judpack anonymously report usage statistics to improve the tool over time?";
    insight.askPermission(msg, function (unused, optIn) {
        var EOL = require('os').EOL;
        if (optIn) {
            console.log(EOL + "Thanks for opting into telemetry to help us improve judpack.");
            track('telemetry', 'on', 'via-cli-prompt-choice', 'successful');
        } else {
            console.log(EOL + "You have been opted out of telemetry. To change this, run: judpack telemetry on.");
